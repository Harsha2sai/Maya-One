...
enum OrbLayoutState { centerDock, floatingRight, floatingLeft, minimizedRight, minimizedLeft, hidden }
...
class OrbController extends ChangeNotifier {
  ...
  void setLifecycle(OrbLifecycle state) {
    if (_lifecycle != state) {
      ...
      } else if (state == OrbLifecycle.appearing)
        _layoutState = OrbLayoutState.centerDock;
  ...
  OrbState get orbState {
    switch (_lifecycle) {
      case OrbLifecycle.listening:
        return OrbState.listening;
      case OrbLifecycle.speaking:
        return OrbState.speaking;
      case OrbLifecycle.initializing:
