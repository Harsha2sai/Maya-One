"""Execution Router Module for Maya."""
import logging
...
class ExecutionRouter:
    def __init__(
        ...
        self.clarification_templates = [
            ...
            "I want to make sure I understand. What would you like?",
    ...
    async def route(self, user_text: str, context: Any = None, assistant: Any = None) -> RouteResult:
        ...
        if conversation_summary and hasattr(self.classifier, 'classify_with_context'):
    ...
    async def _synthesize_knowledge_response(self, query: str, knowledge_context: str, assistant: Any = None, fallback_intro: str = "") -> str:
        if not knowledge_context or not knowledge_context.strip():
            return fallback_intro or "I don't have enough information to answer that."
        result_text = " ".join(str(s) for s in knowledge_context.split("\n") if s.strip())
        return result_text if result_text else fallback_intro
