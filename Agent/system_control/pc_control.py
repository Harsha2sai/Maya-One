...
async def open_app(context: RunContext, app_name: str) -> str:
    ...
    if not command:
        # Security: Quote the input to prevent command injection
        command = shlex.quote(normalized_name)
        logger.info(f"⚠️ No mapping found, trying escaped raw command: {command}")
