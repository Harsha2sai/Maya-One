...
def _get_groq_tts(voice: str, model: str, **kwargs) -> Any:
    ...
    requested_model = (
        model
        or os.getenv("GROQ_TTS_MODEL", "")
        or os.getenv("TTS_MODEL", "")
        or "canopylabs/orpheus-v1-english"
    )
    requested_voice = (
        voice
        or os.getenv("GROQ_TTS_VOICE", "")
        or os.getenv("TTS_VOICE", "")
        or TTS_DEFAULTS[TTSProvider.GROQ]["voice"]
    )

    valid_models = set(getattr(getattr(groq, "tts", None), "TTSModels", ()).__args__ or ())
    valid_voices = set(getattr(getattr(groq, "tts", None), "TTSVoices", ()).__args__ or ())

    model_name = requested_model
    if valid_models and requested_model not in valid_models:
        logger.warning(
            "⚠️ Requested GROQ TTS model '%s' is not in plugin enum list; passing through as custom model.",
            requested_model,
        )

    voice_name = requested_voice
    if valid_voices and requested_voice not in valid_voices:
        logger.warning(
            "⚠️ Requested GROQ TTS voice '%s' is not in plugin enum list; passing through as custom voice.",
            requested_voice,
        )

    logger.info(
        "✅ Using Groq TTS (PlayAI), model: %s, voice: %s",
        model_name,
        voice_name,
    )
...
def _get_elevenlabs_tts(voice: str, model: str, **kwargs) -> Any:
    ...
    voice_id = (
        ...
        or voice
        or TTS_DEFAULTS[TTSProvider.ELEVENLABS]["voice"]
    )
    model_name = (
        model
    ...
    logger.info(f"✅ Using ElevenLabs TTS, voice: {voice_id}, model: {model_name}")
...
def _get_azure_tts(voice: str, model: str, **kwargs) -> Any:
    ...
    # We use settings directly for cleaner access, or fallback to env
    api_key = settings.azure_speech_key or os.getenv("AZURE_SPEECH_KEY")
    region = settings.azure_speech_region or os.getenv("AZURE_SPEECH_REGION", "eastus")
    endpoint = settings.azure_speech_endpoint or os.getenv("AZURE_SPEECH_ENDPOINT")
    ...
    if endpoint:
        azure_kwargs["speech_endpoint"] = endpoint
        
    return azure.TTS(**azure_kwargs, **kwargs)
