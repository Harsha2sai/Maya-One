...
class Settings:
    ...
    # ==================== AZURE SETTINGS ====================
    azure_speech_key: Optional[str] = field(default_factory=lambda: os.getenv("AZURE_SPEECH_KEY"))
    azure_speech_region: str = field(default_factory=lambda: os.getenv("AZURE_SPEECH_REGION", "eastus"))
    azure_speech_endpoint: Optional[str] = field(default_factory=lambda: os.getenv("AZURE_SPEECH_ENDPOINT"))
    ...
    # ==================== LIVEKIT SETTINGS ====================
    livekit_url: Optional[str] = field(default_factory=lambda: os.getenv("LIVEKIT_URL_2") if os.getenv("LIVEKIT_ACTIVE_SLOT") == "2" else os.getenv("LIVEKIT_URL"))
    livekit_api_key: Optional[str] = field(default_factory=lambda: os.getenv("LIVEKIT_API_KEY_2") if os.getenv("LIVEKIT_ACTIVE_SLOT") == "2" else os.getenv("LIVEKIT_API_KEY"))
    livekit_api_secret: Optional[str] = field(default_factory=lambda: os.getenv("LIVEKIT_API_SECRET_2") if os.getenv("LIVEKIT_ACTIVE_SLOT") == "2" else os.getenv("LIVEKIT_API_SECRET"))
    livekit_port: int = field(default_factory=lambda: int(os.getenv("LIVEKIT_PORT", "8082")))
    livekit_agent_name: str = field(default_factory=lambda: os.getenv("LIVEKIT_AGENT_NAME", "maya-one"))
    
    # ==================== ARCHITECTURE PHASE ====================
    # 1 = Pure LiveKit baseline
    # 2 = Reintroduce orchestrator path for text channel and console routing
    # 3 = Reintroduce safe tool pipeline for chat/tool actions
    ...
    # How many consecutive failures before a provider is marked unavailable
    circuit_breaker_threshold: int = field(default_factory=lambda: int(os.getenv("CIRCUIT_BREAKER_THRESHOLD", "5")))
    # How many seconds before a tripped breaker is tested again
    circuit_breaker_recovery_s: float = field(default_factory=lambda: float(os.getenv("CIRCUIT_BREAKER_RECOVERY_S", "30.0")))
    # How many successes needed in HALF_OPEN state to close the circuit
    circuit_breaker_success_threshold: int = field(default_factory=lambda: int(os.getenv("CIRCUIT_BREAKER_SUCCESS_THRESHOLD", "2")))
    ...
    def validate(self) -> bool:
        ...
        if self.llm_provider in llm_key_map:
            ...
            if self.llm_provider == "gemini":
                if not os.getenv(key) and not os.getenv("GEMINI_OAUTH_ACCESS_TOKEN"):
                    errors.append(f"LLM provider '{self.llm_provider}' requires {key} or GEMINI_OAUTH_ACCESS_TOKEN")
        ...
        if self.tts_provider in tts_key_map:
            ...
            if self.tts_provider == "azure":
                if not self.azure_speech_region:
                    errors.append("Azure Speech TTS requires AZURE_SPEECH_REGION")
