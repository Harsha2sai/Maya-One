...
class MCPToolsIntegration:
    ...
    async def prepare_dynamic_tools(mcp_servers: List[MCPServer],
            ...
            for server in mcp_servers:
                if not getattr(server, 'connected', False):
                    try:
                        ...
                        # Set a shorter timeout for the connection attempt if possible
                        await asyncio.wait_for(server.connect(), timeout=10.0)
                    except asyncio.TimeoutError:
                        logger.warning(f"⚠️ Connection timeout for MCP server {server.name}. Skipping.")
                    except Exception as e:
                        logger.warning(f"⚠️ Failed to connect to MCP server {server.name}: {e}. Skipping.")
        ...
        for server in mcp_servers:
            ...
            for tool_instance in mcp_tools:
                # Skip tools that were filtered out due to invalid schemas
    ...
    def _create_decorated_tool(tool: FunctionTool) -> Callable:
        ...
        for p_name, p_details in schema_props.items():
            ...
            # Map boolean to str to be lenient with LLMs that output "true"/"false" strings
            if json_type == "boolean":
                py_type = str
                bool_params.add(p_name)
            else:
        ...
        async def tool_impl(**kwargs):
            ...
            input_json = json.dumps(final_kwargs)
            logger.info(f"Invoking tool '{tool.name}' with args: {final_kwargs}")
