...
class AgentProcessManager {
  ...
  static const String pythonPath = '/home/harsha/Downloads/Projects/v2/Maya-One-phase-0-2/Agent/venv/bin/python';
  static const String agentScriptPath = '/home/harsha/Downloads/Projects/v2/Maya-One-phase-0-2/Agent/agent.py';
  static const String workingDirectory = '/home/harsha/Downloads/Projects/v2/Maya-One-phase-0-2/Agent';
  ...
  Future<bool> startAgent() async {
    ...
    // Avoid spawning a duplicate backend when one is already running.
    if (await _isExternalBackendAvailable()) {
      _log('[AgentManager] Existing backend detected on :5050. Skipping local spawn.');
      return true;
    }
  ...
  Future<bool> _isExternalBackendAvailable() async {
    HttpClient? client;
    try {
      client = HttpClient()..connectionTimeout = const Duration(seconds: 2);
      final request = await client.getUrl(Uri.parse('http://127.0.0.1:5050/health'));
